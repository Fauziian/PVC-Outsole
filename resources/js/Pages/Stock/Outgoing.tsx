...
interface Product { id:number; kategori:string; jenis:string; warna:string; stok_saat_ini:number }
...
export default function Outgoing({transactions,barang_list,filters}:Props) {
  ...
  return <SumberPvcLayout><Head title="Barang Keluar"/>
    ...
    <form onSubmit={save} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4">
      ...
      <div className="space-y-3">{lines.map((line,i)=>{const p=barang_list.find(x=>x.id===Number(line.id_barang));return <div key={i} className="grid grid-cols-1 md:grid-cols-[1fr_150px_120px_1fr_36px] gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
        <select required value={line.id_barang} onChange={e=>update(i,"id_barang",Number(e.target.value))} className="rounded-lg border-slate-200 text-xs">{barang_list.map(x=><option key={x.id} value={x.id}>{x.kategori} — {x.jenis} — {x.warna} ({x.stok_saat_ini} kodi)</option>)}</select>
        ...
        <div><input required min="1" max={p?.stok_saat_ini} type="number" value={line.jumlah} onChange={e=>update(i,"jumlah",Number(e.target.value))} className="w-full rounded-lg border-slate-200 text-xs"/><span className="text-[9px] text-slate-400">Maks. {p?.stok_saat_ini||0} kodi</span></div>
